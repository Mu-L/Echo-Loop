...
> 最后更新：2026-07-12（Onboarding Survey 深色模式视觉修复）
> 当前焦点：Android 结束录音闪退（离线 ASR / Silero VAD）——**仍未解决**

## 已完成：修复 Onboarding Survey 深色模式视觉异常

Onboarding Survey 在夜间模式下仍强制使用浅色页面背景，但题目选项和方法论页部分内容读取深色主题，导致浅色背景、黑色选项卡和浅色文字混用。此次将问卷页面完整接入应用主题，修复所有 Flutter 平台的同类问题。

- [x] **页面主题统一**：移除固定浅色背景，题干、步骤计数、返回按钮和进度条统一使用 `ColorScheme` 语义色。
- [x] **选项状态适配**：普通态、边框、选中背景、文字和勾选图标改用主题 surface / outline / primary container 色。
- [x] **品牌图标可见性**：深色模式下将 GitHub、X、TikTok 等近黑品牌图标回退为主题前景色，避免在 AMOLED 背景上消失。
- [x] **回归测试**：新增深色模式 widget 测试，覆盖题目页、普通/选中选项、近黑品牌图标和 summary 文字颜色。
- [x] **验证**：相关 `flutter analyze` 0 问题；`onboarding_survey_screen_test.dart` 11 项全部通过。

  **完成时间**: 2026-07-12

## 已完成：统一自家后端 API 错误日志

此前只有部分客户端手动安装 `ApiLogInterceptor`，官方 catalog、analytics 等自家后端请求没有统一日志；AI 流式请求又因 `ResponseType.stream + validateStatus:true` 只能在业务层消费错误体，quota 等非 2xx 响应只被映射为异常，没有打印后端 `code/error`。

- [x] **公共工厂统一日志**：`createBackendDio()` 默认安装 `ApiLogInterceptor`，所有经该工厂创建的自家后端客户端自动记录请求、响应、状态码、响应体摘要、耗时和网络异常。
- [x] **业务标签保留**：AI、转录、官方合集、官方 catalog、订阅权益使用各自 tag；移除客户端重复安装的 interceptor，避免同一请求重复打印。
- [x] **流式错误补齐**：翻译、解析、意群和 AI 查词在读完非 2xx 错误 JSON 后记录 endpoint、status、`code/error`，不额外消费响应流。
- [x] **日志脱敏**：递归遮蔽 token、Authorization、用户标识、预签名上传 URL 等敏感字段，长内容继续截断；第三方 CDN/R2/RSS/App Store 请求不接入后端公共工厂。
- [x] **验证**：相关 `flutter analyze` 0 问题；`backend_dio_test.dart`、`api_log_interceptor_test.dart`、`sentence_ai_api_client_test.dart` 共 39 项通过。

  **完成时间**: 2026-07-12

## 已完成：修复 AI 翻译/解析超额后卡加载状态

逐句精听及复用标注组件的页面中，翻译和解析请求遇到登录门槛或 quota 超限时，会持续显示 loading；解析还因 `yield*` 未被外层异常分支捕获而不会进入订阅页。意群链路不等待导航 Future，因此没有同样问题。

- [x] **流异常捕获统一**：解析从 `yield*` 改为 `await for` 转发，确保流内 auth/quota 异常进入标注视图的登录/订阅处理分支。
- [x] **导航与请求解耦**：翻译/解析触发登录或 Paywall 时不等待页面关闭，避免异步按钮被导航 Future 持续占用。
- [x] **卡片状态恢复**：auth/quota 阻断时清理翻译/解析的展开、loading 和未完成内容，按钮立即恢复可点击并允许重试。
- [x] **回归测试**：补充翻译/解析 quota 后恢复 idle、允许重试，以及两类请求立即进入 Paywall 的 widget 测试。
- [x] **验证**：相关 `flutter analyze` 0 问题；`annotation_content_view_auth_test.dart` 与 `sentence_annotation_card_test.dart` 共 33 项通过。

  **完成时间**: 2026-07-12

## 已完成：平台 + 渠道统一识别

支付入口继续由客户端本地裁决，后端只使用统一 identity 对 AI/转录限额做渠道灰度。正式协议为平台 `android | ios | macos | windows` 与渠道 `play | app_store | direct`；旧客户端缺少 distribution 时继续 fail-open。

- [x] **客户端 identity**：新增强类型 distribution 解析层，旧 `apk/desktop` 兼容映射为 `direct`，公共请求头增加 `x-app-distribution`。
- [x] **支付本地裁决**：订阅可用性、Web checkout、购买服务和管理订阅 URL 统一基于合法的 platform + distribution 组合。
- [x] **后端组合限额**：新增 client identity 规范化模块，限额名单升级为 `AI_QUOTA_ENFORCED_CLIENTS`，9 条 AI/转录链路统一接入；缺失、非法值或非法组合 fail-open。
- [x] **发布注入**：Android APK=`direct`、AAB=`play`、iOS=`app_store`、macOS DMG=`direct`；仓库当前没有 Windows/macOS App Store 发布脚本。
- [x] **验证**：Flutter 相关 analyze 0 问题、定向测试 61 项通过，带 `DISTRIBUTION_CHANNEL=direct` 的 header 测试 12 项通过；后端 quota + route 测试 96 项、Next.js app typecheck 通过。`scripts/check.sh` 已执行：全量 analyze 通过，全量测试运行 3945 项后有 49 个既有失败（迁移测试缺 `sentence_ai_cache` 等），因此未进入 integration/macOS build；`@repo/payments` typecheck 另被既有 design-system SVG 文件缺失阻断。

  **完成时间**: 2026-07-12

## 已完成：调整随心听主控制按钮间距

随心听页面底部上一句/播放/下一句三个主控制按钮之间仅 12px 间距，触控区靠得过近且与其它学习播放页不一致。本次将随心听主控制行收敛到学习页同款布局：三个主按钮统一 56px 固定槽位，左右按钮与中间主播放按钮间距统一为 48px；无字幕场景的后退/前进 10 秒按钮也复用同一主控制间距。

- [x] **主控制间距统一**：`PlaybackControls` 的上一句/播放/下一句、后退/播放/前进行统一使用 56px 按钮槽 + 48px 主间距。
- [x] **触控稳定**：左右导航按钮和中间播放按钮都固定尺寸，避免图标/状态变化影响布局。
- [x] **回归测试**：补充移动端控制栏中心距断言，锁定按钮中心距为 104px（56px 槽位 + 48px 间距）。
- [x] **验证**：`flutter analyze` 相关文件 0 问题；`playback_controls_test.dart` 与 `player_screen_test.dart` 全过。

  **完成时间**: 2026-07-12

## 已完成：修复讲解页返回播放器误播与按钮状态错误

从自由播放器正文进入句子讲解页并播放意群后，返回播放器会短暂漏播当前句开头，且主播放按钮误显示暂停图标。根因是讲解页/意群试听旁路驱动共享 `AudioEngine` 后，底层 `playing=true` 残留；播放器返回时直接 `seek` 到当前句句首，等同于在播放中跳转，因此漏出一小段声音并污染逻辑播放态。本次让播放器恢复位置前先暂停并失效外来 session，再清 clip/seek；讲解页 dispose 清理从延迟 `stop` 改为延迟 `pause`，避免晚到的 stop 重置返回后的 clip/position。

- [x] **恢复路径防漏播**：`ListeningPractice._alignEngineToCurrent()` 先 `pause()` 再 `clearClip()` / `seek()`，保证 `restorePosition()` 只对齐位置、不隐式起播。
- [x] **讲解页清理收敛**：`SentenceDetailScreen.dispose()` 改为延迟 `pause()`，停止讲解页试听但不在播放器返回后重置位置。
- [x] **回归测试**：补充 session guard 测试，锁定外来播放残留时 `restorePosition()` 会先暂停、保持 `isPlaying=false` 并 seek 到当前句首。
- [x] **验证**：`flutter analyze` 相关文件 0 问题；`session_guard_test.dart` 与 `player_screen_test.dart` 全过。

  **完成时间**: 2026-07-12

## 已完成：意群快捷 AI lookup

意群 badge 的快捷操作从单一收藏扩展为「收藏 + AI lookup」。lookup 按钮位于收藏右侧，点击后用当前意群文本打开现有非 modal 词典面板，并强制本次查询初始选中 AI 源；快捷条不立即关闭，继续自然消失，便于用户查词后继续收藏。查词面板打开期间 lookup 按钮锁定不可重复点击，面板关闭后自动恢复。

- [x] **快捷条扩展**：`SenseGroupActionBar` 增加右侧 AI lookup 图标按钮，快捷条改为胶囊浮层、轻边框、柔和阴影和低对比分隔线，并单独提升夜间模式对比度。
- [x] **AI 源指定**：`DictionaryPanelQuery` 支持 `preferredSourceId`，lookup controller 本次查询优先使用显式源，不污染普通点词的默认源/会话粘滞源规则。
- [x] **重复点击防护**：`DictionaryPanelHost` 暴露面板开关监听，意群 lookup 在查词面板打开期间禁用，关闭面板后解锁；收藏按钮不受影响。
- [x] **验证**：`flutter analyze` 相关文件 0 问题；lookup controller、快捷条、词典面板、可点词相关 widget 测试全过。

  **完成时间**: 2026-07-12

## 已完成：AI 流式响应帧调试日志

为排查 `/api/v1/stream/*` 流式 AI 响应，补齐客户端逐帧日志。Dio 拦截器仍不直接消费 `ResponseBody`，避免破坏业务解析；改为在 NDJSON 解码入口打印每条非空原始响应行，覆盖翻译、句子解析、AI 查词和意群拆分流式端点。单帧日志超过 2000 字符会截断，日志回调异常不影响流式解析。

- [x] **流式帧打印**：`SentenceAiApiClient` 在解码 NDJSON 时输出 `流式响应帧 METHOD URL: ...`，保留原始 `ops` / `done` / `__error` 内容。
- [x] **安全旁路**：`decodeNdjson` 新增可选 `onLine` 回调，只读已被业务消费的原始行，不额外消费响应流。
- [x] **验证**：`flutter analyze` 相关文件 0 问题；`flutter test test/services/ndjson_stream_test.dart test/services/sentence_ai_api_client_test.dart` 全过。

  **完成时间**: 2026-07-12

## 已完成：修复播放器句子正文点击后返回焦点错误

自由播放器全文/收藏句子列表中，正文点击用于进入句子讲解页，左侧编号区用于从该句播放。此前正文点击不会更新播放器当前句，导致从第 21 句点击第 28 句进入讲解页后，返回列表仍按旧当前句自动滚回第 21 句。本次让正文点击在导航前先同步当前句焦点，但保持 `autoPlay: false`，不改变“只有编号区负责播放”的交互边界。

- [x] **正文点击同步焦点**：`PlayerScreen` 在进入句子讲解页前按当前 playlist mode 调用 `selectFullSentence` / `selectBookmarkedSentence`，只切换焦点不自动播放。
- [x] **回归测试**：加强播放器 widget 测试，锁定点击非当前句正文会以 `autoPlay:false` 同步当前句并进入讲解页。
- [x] **验证**：`flutter analyze lib/screens/player_screen.dart test/screens/player_screen_test.dart` 0 问题；`flutter test test/screens/player_screen_test.dart` 全过。

  **完成时间**: 2026-07-12

## 已完成：AI API 启用 HTTP/2 访问层

仅对 `SentenceAiApiClient` 的 AI API 访问层启用 HTTP/2，覆盖翻译、句子解析流、意群拆分与 AI 查词流；转录 API 与 R2 上传不变。HTTPS API 默认使用 `dio_http2_adapter`，本地 `http://localhost` 与显式 `AI_HTTP2_ENABLED=false` 保持 Dio 默认 adapter，便于稳定回退。AI API 日志会打印实际 HTTP 版本，并记录 HTTP/2 新建/重建连接和 adapter 主动关闭。

- [x] **AI transport 收敛**：新增 AI 专用 Dio adapter 配置，HTTPS 默认启用 HTTP/2，空闲连接 30 秒；HTTP、本地开发与 Web fallback 保持默认 adapter。
- [x] **回退开关**：新增 `--dart-define=AI_HTTP2_ENABLED=false` kill switch，避免线上网关或平台兼容性问题需要重新发版才能回退。
- [x] **观测日志**：`ApiLogInterceptor` 在响应行打印 `HTTP/2.0` 等实际协议版本；`AI-HTTP2` tag 打印 `connect start` / `reconnect start` / `adapter close`。
- [x] **验证**：`flutter analyze` 相关文件 0 问题；`flutter test test/services/api_log_interceptor_test.dart test/services/sentence_ai_api_client_test.dart` 全过。

  **完成时间**: 2026-07-11

## 已完成：句子解析流式接收与缓存失效

句子解析改为流式结构化协议后，旧 AI 解析缓存不再兼容。本次修复让 app 只清理句子解析相关缓存，保留查词、翻译、意群等仍然有效的缓存；同时补齐流式请求去重、取消和 PDF 导出读取新结构化缓存的测试覆盖。

- [x] **解析缓存失效**：数据库 schema 升到 v45，迁移时清理 `analysis:%` 与 `analysis_v2:%`，保留 `translation:%`、`ai_dictionary`、`sense_groups`。
- [x] **流式请求去重与取消**：同一句同语言的并发解析复用一条底层流式 API 请求；UI 重试和销毁时传递并取消 `CancelToken`。
- [x] **PDF 导出适配**：PDF loader/test 改为读取 `analysis_v2` 结构化解析缓存，不再依赖旧 wrapper 格式。
- [x] **验证**：`flutter analyze` 相关文件 0 问题；相关单测 135 条全过。

  **完成时间**: 2026-07-11

## 已完成：移除流式 AI 词典 queryType 协议字段

流式 AI 词典前后端已拆成 `/lookup-word` 与 `/lookup-phrase` 两个独立端点，`queryType` 协议字段彻底移除；单词/词组作为两条完全独立的流式功能，类型由「调用哪个端点/查询是否含空白」决定，`originalExpression` 在客户端不再参与任何类型判定（仅作多词模型的展示字段）。

- [x] **移除 queryType 字段**：删除 `AiDictionaryQueryType` enum、`AiDictionaryEntry.queryType` getter，多词/单词 `toJson` 不再写 `queryType`。
- [x] **移除结构嗅探分派**：删除 `AiDictionaryEntry.fromJson` sniffer factory；单词端点直传 `DictionaryEntry.fromJson`、词组端点直传 `MultiWordDictionaryEntry.fromJson`（`_streamDictionaryFrames` 参数化 `fromJson`）；L2 缓存读按 `isPhrase` 选具体类型。渲染层仍按 Dart 类型 `switch` 穷尽分派。
- [x] **流式协议收敛**：`accumulateNdjsonObject` 删除 `meta`（元信息帧重命名）机制，解析直接从 `ops`/`done`/`__error` 开始。
- [x] **验证**：`flutter analyze` 相关文件 0 问题；相关单测 56 条全过。
