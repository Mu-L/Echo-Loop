...
- 📋 **Phase 1 — 后端（Next.js 仓库）**：统一 AI 端点鉴权化（C2：词解端点
  ...
  - ✅ **平台+渠道统一识别（2026-07-12）**：客户端以 `platform + distribution` 本地选择 Apple IAP、
    Google Play 或 Web/Paddle，正式 distribution 为 `play | app_store | direct`，旧 `apk/desktop`
    仅兼容映射为 direct。后端以 `AI_QUOTA_ENFORCED_CLIENTS` 按合法组合灰度限额；请求缺失或非法
    distribution 时 fail-open。`/api/entitlements` 仍只返回用户权益。见 `docs/subscription-setup.md` §4/§7。
