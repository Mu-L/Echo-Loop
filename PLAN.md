...
- AI 对话助手（通用 chatbot 组件）：多轮对话式 AI 助手，一套可插拔组件接入不同位置（首接入点为句子讲解页），复用现有 NDJSON 流式与 402 额度门链路。发布由 `kChatbotEnabled`（默认 false）控制，后端流式端点就绪后翻开关启用。规格见 [docs/chatbot-implementation-plan.md](./docs/chatbot-implementation-plan.md)。
